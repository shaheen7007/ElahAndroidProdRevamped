...
//	private static final String KEY_SALESPERSON_CODE = "sales_code";
	...
	public void onClick(View v) {
		if(v==mBtnCopySurvey){
			if(mCopyTask==null){
				mCopyTask = new GetPreviousSurveyTask(this);
...
//				Util.showToast(mToast, R.string.error_server);
	...
	private class GetPreviousSurveyTask extends AsyncTask<String, Integer, Long>{
		...
		private GetPreviousSurveyTask(Context context){
			this.mContext = context;
		}
		...
		protected Long doInBackground(String... params) {
			...
			try {
				...
				surveyId = TableSurvey.createNewSurvey(new Survey(salesPersonCode, customerCode,TableSurvey.FLAG_FROM_SERVER, Util.getCurrentDateTime(),1,0));
